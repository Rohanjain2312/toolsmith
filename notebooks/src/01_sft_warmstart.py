...
# Only "role": "tool" turns carry a "tool_name" field -- most rows in both files don't have it,
# so leaving schema inference to load_dataset is order-dependent: it locks in a schema from
# whichever batch it reads first, then fails to cast a later batch that has the extra field.
# Declaring the schema explicitly avoids that: every row gets tool_name=null unless it's a tool
# turn, regardless of file order.
...
dataset: Dataset = load_dataset(
    "json", data_files=DATA_PATHS, split="train", features=MESSAGE_FEATURES
).shuffle(seed=3407)
