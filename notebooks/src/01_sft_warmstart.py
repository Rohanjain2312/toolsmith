...
# Gold trajectories include "role": "tool" turns with a "tool_name" field that the public data
# never has -- load both files in a single load_dataset call (not one call per path) so the JSON
# builder infers one merged schema across all rows instead of a mismatched schema per file, which
# fails to concatenate.
dataset: Dataset = load_dataset("json", data_files=DATA_PATHS, split="train").shuffle(seed=3407)
