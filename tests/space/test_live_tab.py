...
def test_run_live_task_yields_progressively_then_final(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    updates = list(run_live_task("Geocode Paris.", real_mode=False))
...
def test_run_live_task_final_answer_only_yields_once(monkeypatch: pytest.MonkeyPatch) -> None:
    model = StubModel(["No tool needed, the answer is 42."])
    monkeypatch.setattr(live_tab, "_build_model", lambda: model)

    updates = list(run_live_task("What is 6*7?", real_mode=False))
