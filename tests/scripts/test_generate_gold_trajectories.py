...
from scripts.generate_gold_trajectories import generate_gold_trajectory, load_train_tasks
...
def test_generate_gold_trajectory_passes_goal_check(tmp_path: Path) -> None:
    ...
    model = StubModel(['{"tool": "geocode_city", "args": {"city": "Paris"}}', "Paris is at 48.8N."])

    passed = generate_gold_trajectory(spec, model, trajectory_dir=tmp_path)
...
def test_generate_gold_trajectory_fails_goal_check_when_wrong_tool_called(tmp_path: Path) -> None:
    ...
    model = StubModel(['{"tool": "geocode_city", "args": {"city": "Tokyo"}}', "Tokyo is nice."])
